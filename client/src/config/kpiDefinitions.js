...
export const KPI_MEASURES = KPI_CATEGORIES.flatMap((cat) => cat.measures.map((m) => ({ ...m, categoryKey: cat.key })));
...
export function measureScore(def, data) {
  if (!data) return null;
  if (def.type === 'check') {
...
export function computeCategoryScore(category, measuresData) {
...
export function computeOverallScore(measuresData) {
